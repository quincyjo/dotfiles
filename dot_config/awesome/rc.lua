...
--- Compositor features.
local CompositorFeatures = {
	Shadow = "_PICOM_SHADOW",
	Fade = "_PICOM_FADE",
	Rounded = "_PICOM_ROUNDED",
}

--- Sets the given property of the given window to the given value.
---@param window string
---@param props table<string, string | nil>
---@param default? string
local function set_window_props(window, props, default)
	for prop, value in pairs(props) do
		local arg = string.format("-f %s 32c -set %s %s", prop, prop, value or default or "0")
		awful.spawn(string.format("xprop -id %s %s", window, arg), false)
	end
end

--- Enables the provided compositor features for the given window.
---@param window string
---@param ... string
local function enable_compositor(window, ...)
	local flags = {}
	for _, feature in ipairs({ ... }) do
		flags[feature] = "1"
	end
	set_window_props(window, flags)
end

--- Disables the provided compositor features for the given window.
---@param window string
---@param ... string
local function disable_compositor(window, ...)
	local flags = {}
	for _, feature in ipairs({ ... }) do
		flags[feature] = "0"
	end
...
screen.connect_signal("arrange", function(s)
	...
	disable_compositor(s.mywibox.window, CompositorFeatures.Rounded, CompositorFeatures.Fade)
	for _, c in pairs(s.clients) do
		if not c.floating then
			num_tiled_clients = num_tiled_clients + 1
			if num_tiled_clients > 1 then
				-- We only care about if there is more than 1 tiled client.
				break
			end
		end
	end
	for _, c in pairs(s.clients) do
		-- If this is the only tiles client or it is maximized, no border.
		-- For floating clients, or multiple tiled clients, enable border.
		if layout == "max" or num_tiled_clients == 1 and not c.floating or c.maximized then
			c.border_width = 0
		else
			c.border_width = beautiful.border_width
		end
		-- Floating windows get a shadow.
		if layout == "floating" or c.floating and not c.maximized then
			enable_compositor(c.window, CompositorFeatures.Shadow, CompositorFeatures.Fade, CompositorFeatures.Rounded)
		else
			disable_compositor(c.window, CompositorFeatures.Shadow, CompositorFeatures.Fade, CompositorFeatures.Rounded)
		end
	end
end)
...
awful.screen.connect_for_each_screen(function(s)
	...
	-- Because Picom works by exluding windows from features, we explicitly set
	-- both enabled and disabled feature.
	disable_compositor(s.mywibox.window, CompositorFeatures.Rounded, CompositorFeatures.Fade)
	enable_compositor(s.mywibox.window, CompositorFeatures.Shadow)
end)
...
globalkeys = mytable.join(
	...
	awful.key({ modkey }, "w", function()
		awful.util.mymainmenu:show({ coords = { x = 0, y = 19 } })
	...
	awful.key({ modkey }, "`", function()
		awful.spawn("kitten quick-access-terminal")
	end, { description = "quake terminal", group = "launcher" }),
	...
	awful.key({ modkey, altkey }, "h", function()
		awful.spawn.with_shell(terminal .. ' --title "File System" vifm $HOME')
	end, { description = "show filesystem", group = "widgets" }),
	awful.key({ "Control", "Shift" }, "h", function()
		awful.spawn(terminal .. ' --title "Top" btop')
	end, { description = "show processes", group = "widgets" }),
	...
	end, { description = "screen snippet", group = "widgets" }),
...
clientkeys = mytable.join(
	...
	awful.key({ modkey, "Control" }, "y", awful.placement.centered, { description = "center client", group = "client" }),
